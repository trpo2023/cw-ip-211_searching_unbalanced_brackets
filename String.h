...
///
/// A String class without many of the inconsistencies that `std::string`
/// brings, and many helper functions, making it more alike Python or .NET
/// strings.
///
/// Implemented as a wrapper around a `std::vector<char>` for safety, simplicity
/// and speed. This means that any `<algorithm>` calls should work as expected.
///
/// \attention String is \b not null-terminated. If a null-terminated string is
/// needed, it's very simple to convert to a `std::string` or c-string via
/// `String::to_c_string` and `String::to_std_string`.
class String {
private:
...
public:
    ...
    Iterator end();
    /// \brief Const end iterator. points at the position past the end of the
    /// string.
    ...
    /// \brief A unique_ptr managed char[] containing a copy of the data of the
    /// string, guaranteed to be null-terminated.
    std::unique_ptr<char[]> to_c_string() const;
    /// \brief A copy of this string represented as a std::string.
    std::string to_std_string() const;

    /// \brief Clears the contents of the string, resulting string will be the
    /// empty string.
    void clear() noexcept;

    /// \brief Inserts a char before the position pointed to by the iterator.
    /// May invalidate iterators.
    void insert(ConstIterator iter, char c);
    /// \brief Inserts the string before the position pointed to by the
    /// iterator. May invalidate iterators.
    void insert(ConstIterator iter, const String& s);
    /// \brief Inserts the part of the string specified by the begin and end
    /// iterators before the position pointed to by the "iter" iterator. May
    /// invalidate iterators.
    void insert(ConstIterator iter, ConstIterator begin, ConstIterator end);

    /// \brief Removes the element pointed to by the iterator. Invalidates
    /// iterators.
    ...
    /// \brief Removes n elements starting at the iterator position. Invalidates
    /// iterators.
    ...
    /// \brief Finds the first occurance of char c in the string. Returns end()
    /// if nothing was found. \arg `c` character to find, case-sensitive.
    /// \return String::Iterator pointing to the found character, or end() if
    /// nothing was found.
    Iterator find(char c);
    /// \brief Finds the first occurance of char c in the string. Returns end()
    /// if nothing was found. \arg `c` character to find, case-sensitive.
    /// \return String::ConstIterator pointing to the found character, or end()
    /// if nothing was found.
    ConstIterator find(char c) const;
    /// \brief Finds the first occurance of char c in the string after `start`.
    /// Returns end() if nothing was found. \arg `c` character to find,
    /// case-sensitive. \arg `start` position from which to search from. \return
    /// String::Iterator pointing to the found character, or end() if nothing
    /// was found.
    Iterator find(char c, Iterator start);
    /// \brief Finds the first occurance of char c in the string after `start`.
    /// Returns end() if nothing was found. \arg `c` character to find,
    /// case-sensitive. \arg `start` position from which to search from. \return
    /// String::ConstIterator pointing to the found character, or end() if
    /// nothing was found.
    ConstIterator find(char c, ConstIterator start) const;

    /// \brief Finds the first occurance of the string inside this string.
    /// \return String::Iterator pointing to the beginning of the found
    /// substring, or end() if nothing was found
    Iterator find(const String&);
    /// \brief Finds the first occurance of the string inside this string.
    /// \return String::ConstIterator pointing to the beginning of the found
    /// substring, or end() if nothing was found
    ConstIterator find(const String&) const;
    /// \brief Finds the first occurance of the string after `start` inside this
    /// string. \return String::Iterator pointing to the beginning of the found
    /// substring, or end() if nothing was found
    Iterator find(const String&, Iterator start);
    /// \brief Finds the first occurance of the string after `start` inside this
    /// string. \return String::ConstIterator pointing to the beginning of the
    /// found substring, or end() if nothing was found
    ...
    /// \brief Does a case-sensitive comparison between the chars of both
    /// strings. Same as String::operator==(const String&).
    ...
    /// \brief Replaces \b all instances of `to_replace` with `replace_with` in
    /// the string.
    void replace(char to_replace, char replace_with);
    /// \brief Replaces \b all instances of `to_replace` with `replace_with` in
    /// the string.
    void replace(const String& to_replace, const String& replace_with);
    /// \brief Replaces `n` instances of `to_replace` with `replace_with` in the
    /// string.
    void
    replace(const String& to_replace,
    ...
    /// \arg `expected_splits` how many parts are expected. Setting this to a
    /// reasonable amount will speed up the split operation as memory can be
    /// reserved beforehand.
    std::vector<String>
    split(char delim, std::size_t expected_splits = 2) const;
    /// \brief Splits the String into substrings delimited by the String
    /// `delim`.
    ///
    ...
    /// reserved beforehand.
    std::vector<String>
    split(const String& delim, std::size_t expected_splits = 2) const;

    /// \brief Grows the capacity to fit `size` many characters. Does not change
    /// the size of the string.
    ///
    /// Increases the capacity of the currently allocated memory to be able to
    /// hold `size` many characters before having to reallocate. Calling this
    /// before doing a lot of appending up to a known max length can increase
    /// performance significantly, as multiple allocations are avoided. Does not
    /// impact length.
    void reserve(std::size_t size);

    /// \brief Capacity of the string's underlying allocated memory.
    ///
    /// The String's size may grow up to this capacity without any reallocation
    /// taking place.
    std::size_t capacity() const;

    /// \brief Shrinks capacity to be equal to size, thus freeing memory in some
    /// cases. Not guaranteed, as it's implementation dependent.
    ...
    struct Format {
        ...
        /// \brief The precision (i.e. how many digits are generated) of
        /// floating point numbers output with this format. See
        /// `std::ios::precision`.
        int precision{6};
        /// \brief The base used to display integer types. See `std::ios::base`.
        Base base{Base::Dec};
        /// \brief Alignment used with width. See `std::ios::left` and
        /// `std::ios::right`.
        Align alignment{Align::Left};
        /// \brief Width used on some format operations. See `std::ios::width`.
        int width{0};
        /// \brief Filler used to fill whitespace in width formats. See
        /// `std::ios::fill`.
        char fill{' '};
...
/// \brief Null-terminated, fully constexpr string that will almost completely
/// disappear with compiler optimizations turned on.
///
/// To be used for string constants, as it's as fast as declaring a `const
/// char*` but knows its own size and can be compared with operator==, which is
/// optimized into constants at compile-time.
class ConstString {
private:
    const char* m_buffer;
...
public:
    ...
    constexpr ConstString(const char*&& buffer)
        : m_buffer(buffer), m_size(length(buffer))
    {
    }
    ...
    bool operator==(const String& str) const
    {
        return m_size != str.size()
                || std::strncmp(m_buffer, str.data(), m_size) == 0;
    }
};
